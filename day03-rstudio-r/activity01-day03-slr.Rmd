...
2. What type of plot would you use to display the distribution of the personal freedom scores, `pf_score`? 
  ...
  A: To look at the distribution of `pf_score`, I would use a histogram.
...
4. Comment on each of these two distributions. 
  ...
  - There are potential outliers in the tail where `pf_score` is between 2 and 3.
  - The center of the data/peak of the histogram is at approximately 6.5.
  - Spread of the data is between values 2 and 10.
  
  A: pf_expression_control
  - Spread is between 0 and 8. 
  - Increments in 1.25.
  - data peaks around 4.
  - Roughly unimodal in shape.
...
A: I would use a scatterplot to display the relationship between `pf_score`
...
4. Does the relationship look linear?
  ...
  A: The relationship in the above scatterplot does look linear. If I recall correctly, certain conditions need to be met before we attempt a linear regression model. 
...
When `pf_expression_control` equals 0, we can expect `pf_score` to be 4.28. 


For every1 unit increase in `pf_expression_control`, we can expect `pf_score` to increase 0.54 units.
...
9. If someone saw the least squares regression line from (5) and not the actual data, how would they predict a country's personal freedom school when their `pf_expression_control` is an index of 3?
  ...
  I imgain they would predict a `pf_score` of about 6.2 or 6.25 if the `pf_expression_control` is an index of 3.
...
Yes, the residuals appear to be approximately normal in their distribution (there is a slight skew to the left).
...
The constant variability condidion appears to be met. There is no discernable shape in the residuals vs. fitted plot (snowstorm!).
